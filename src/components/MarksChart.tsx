...
export type Attempt = { subject_id: string; year: number | null; created_at: string; percentage: number | null }
export default function MarksChart({ attempts }: { attempts: Attempt[] }) {
  // Build time series by attempt order (or by date)
  const data = useMemo(() => {
    const rows = (attempts || [])
      .filter(a => typeof a.percentage === "number")
      .map((a, i) => ({ i, date: new Date(a.created_at).toLocaleDateString(), pct: Number(a.percentage) }))
    return rows
  }, [attempts])
