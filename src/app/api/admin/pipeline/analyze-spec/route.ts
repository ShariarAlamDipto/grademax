...
export async function POST(req: NextRequest) {
  ...
  const { specText, subjectName, level } = await req.json()
...
Return ONLY the JSON array, no explanation.`
  ...
  try {
    ...
    try {
      ...
      topics = JSON.parse(trimmed.slice(jsonStart, jsonEnd))
    } catch {
      return NextResponse.json({ error: "Failed to parse LLM response as JSON", raw: text }, { status: 500 })
    }
