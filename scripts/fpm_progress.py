#!/usr/bin/env python3
"""
Check FPM classification progress
"""

import os
import sys
from pathlib import Path
from dotenv import load_dotenv
from supabase import create_client

# Load environment
env_path = Path(__file__).parent.parent / '.env.local'
load_dotenv(env_path)

supabase = create_client(
    os.getenv("NEXT_PUBLIC_SUPABASE_URL"),
    os.getenv("SUPABASE_SERVICE_ROLE_KEY")
)

print("\n" + "="*80)
print("📊 FPM CLASSIFICATION PROGRESS")
print("="*80)

# Get FPM subject
fpm = supabase.table("subjects").select("*").eq("code", "9FM0").execute()
if not fpm.data:
    print("❌ FPM subject not found")
    sys.exit(1)

fpm_id = fpm.data[0]['id']
print(f"\n✅ Subject: {fpm.data[0]['name']} ({fpm.data[0]['code']})")

# Get paper count
papers_response = supabase.table("papers").select("id", count="exact").eq("subject_id", fpm_id).execute()
paper_count = papers_response.count if papers_response.count else 0
print(f"📄 Papers in database: {paper_count}")

if not papers_response.data:
    print("❌ No papers found")
    sys.exit(0)

paper_ids = [p['id'] for p in papers_response.data]

# Get total pages
all_pages = supabase.table("pages")\
    .select("id", count="exact")\
    .in_("paper_id", paper_ids)\
    .execute()
total_pages = all_pages.count if all_pages.count else 0
print(f"📝 Total pages: {total_pages}")

# Get classified pages (topics array not empty)
classified = supabase.table("pages")\
    .select("id", count="exact")\
    .in_("paper_id", paper_ids)\
    .neq("topics", "{}")\
    .execute()
classified_count = classified.count if classified.count else 0

print(f"✅ Classified pages: {classified_count}")
print(f"⏳ Remaining: {total_pages - classified_count}")

if total_pages > 0:
    progress = (classified_count / total_pages) * 100
    print(f"📊 Progress: {progress:.1f}%")

# Get topic distribution
print(f"\n📊 Topic Distribution:")
for i in range(1, 11):
    topic_pages = supabase.table("pages")\
        .select("id", count="exact")\
        .in_("paper_id", paper_ids)\
        .contains("topics", [str(i)])\
        .execute()
    count = topic_pages.count if topic_pages.count else 0
    if count > 0:
        print(f"   Topic {i}: {count} pages")

print()
