#!/usr/bin/env python3
"""
Phase 2 for the IAL Pure Mathematics 4 (WMA14) chapterwise workbook: turn the Phase 1
segments into classifier-ready question records.

    python scripts/enrich_p4_workbook_questions.py            # dry run
    python scripts/enrich_p4_workbook_questions.py --execute

Reads `data/workbook/p4/*/manifest.json`, writes
`data/workbook/p4_questions.json`. Nothing else is touched, and re-running is
...
**The bands are P4's own and must not be copied from another subject.** Measured
over these 130 questions: mean 8.1, median 8, range 2-15. P4 questions run at
roughly two thirds of an S1 question's tariff, so S1's cut points (easy <= 10)
would call 55 of 130 P4 questions easy and only 26 hard. The cut points below
are the most even three-way split of P4's actual distribution.
"""
...
SEGMENT_DIR = REPO_ROOT / "data" / "workbook" / "p4"
OUTPUT_PATH = REPO_ROOT / "data" / "workbook" / "p4_questions.json"

UNIT = "P4"
SUBJECT_CODE = "WMA14"

#: Measured from this subject's own distribution (see the module docstring).
#: easy <= 6 | medium 7-8 | hard >= 9  ->  36 / 40 / 54  (27% / 30% / 41%)
EASY_MAX = 6
MEDIUM_MAX = 8
...
def main() -> int:
    ...
    if not manifests:
        print(f"no manifests under {SEGMENT_DIR} -- run build_p4_workbook_segments.py --execute first")
        return 1

    mode = "EXECUTE" if args.execute else "DRY RUN"
    print(f"=== P4 ({SUBJECT_CODE}) enrichment  [{mode}] ===\n")
                ...
                records.append(
                    {
                        ...
                        "difficulty": band,
                        "qp_pages": list(pages),
                        "qp_pdf": f"p4/{manifest['key']}/questions/q{number}.pdf",
                        "stem": stem,
                        "stem_chars": len(stem),
                        "text_status": status,
                        "sub_parts": (
