#!/usr/bin/env python3
"""
Phase 3 for P4 (WMA14): group classified questions into recurring archetypes.

    python scripts/cluster_p4_archetypes.py              # dry run
    python scripts/cluster_p4_archetypes.py --execute
    python scripts/cluster_p4_archetypes.py --threshold 0.38   # to compare

Reads `p4_questions.json` and `p4_classification_cache.json`, writes
`p4_archetypes.json`. Deterministic: no model is involved, so a re-run after
...
OUTPUT_PATH = REPO_ROOT / "data" / "workbook" / "p4_archetypes.json"
#: 0.32, set by reading the clusters: it merges section 4.1's four
#: separate binomial clusters into the one shape they plainly are, while
#: keeping 7.2 from swallowing its whole section. See lib/ial_archetypes.py.
THRESHOLD = 0.32
