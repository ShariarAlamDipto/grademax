"""
Phase 3b: cluster Maths B workbook questions into recurring archetypes.

Edexcel reuses question shapes heavily -- "express a sin x + b cos x in the form
R sin(x + a), then solve in a given range" recurs most years with different
numbers. The workbook keeps every repeat (the brief is that no variety is left
...
with no memory of the others, so 431 questions produce ~431 distinct phrasings
of maybe 120 real shapes. Clustering has to be done here, over the whole corpus
at once.
...
USAGE
-----
    python scripts/cluster_mathsb_archetypes.py
    python scripts/cluster_mathsb_archetypes.py --threshold 0.4
    python scripts/cluster_mathsb_archetypes.py --execute
    python scripts/cluster_mathsb_archetypes.py --show 9.4
"""
...
OUTPUT_PATH = REPO_ROOT / "data" / "workbook" / "mathsb_archetypes.json"
...
def main() -> int:
    ...
    if not CACHE_PATH.is_file():
        print("No classifications yet. Run classify_mathsb_workbook_sections.py first.")
        return 1
