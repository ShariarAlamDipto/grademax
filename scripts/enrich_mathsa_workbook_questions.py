"""
Enrich the segmented Maths A (4MA1) Higher questions with everything Phase 3
needs to classify them and Phase 5 needs to print them.

Reads the Phase 1 output (data/workbook/mathsa/) and writes a single inventory
at data/workbook/mathsa_questions.json: marks, difficulty, sub-parts, and the
question stem as plain text.

Full working notes: docs/workbook-mathsa-plan.md

DIFFICULTY IS NOT COPIED FROM EITHER PREDECESSOR
------------------------------------------------
Maths A questions are SHORT: measured over all 1,042 questions the mean is 4.13
marks, the median 4, and nothing exceeds 9. Neither earlier band set survives
that. FPM's (<=6 easy, <=10 medium) would call 94% of this book easy, and Maths
B's (<=3, <=6) leaves only 6% hard. Bands here come from this subject's own
distribution:

    <=3 marks   easy     ~39%   (81 + 320 questions)
    4-5 marks   medium   ~46%   (262 + 216)
    >=6 marks   hard     ~15%   (99 + 36 + 17 + 5)

Banding globally rather than per-paper is deliberate, and costs nothing here:
unlike Maths B, 1H and 2H have the same shape, so neither paper is pushed into
one band.
...
of 0.93 -- confident and useless. The tariff is what the examiner assigned.

TEXT REPAIR
-----------
Kept from the Maths B enricher: some Edexcel font subsets carry a broken
ToUnicode CMap where character codes sit 29 below their true value. The repair
is applied PER SPAN, never per page -- font metadata cannot separate broken
spans from intact ones, and decoding a whole page corrupts everything already
correct. A repair also has to earn its place by reading better than the original.

Note this corpus's own text problem is DIFFERENT and not repairable here: the
four 2019 Jan papers contain image-only pages with no text layer at all, so the
questions that live on them get `needs_vision` rather than a repair.

`partially_garbled` must NOT reach the database: migration 12's CHECK allows
only ('ok','repaired','needs_vision'), and a dry run never attempts an insert so
it cannot catch that. The loader maps it to `repaired`.

USAGE
-----
    python scripts/enrich_mathsa_workbook_questions.py
    python scripts/enrich_mathsa_workbook_questions.py --execute
    python scripts/enrich_mathsa_workbook_questions.py --show 2022_jan_1H 7
"""
...
WORKBOOK_DIR = REPO_ROOT / "data" / "workbook" / "mathsa"
OUTPUT_PATH = REPO_ROOT / "data" / "workbook" / "mathsa_questions.json"

# See the module docstring -- derived from this corpus's own 2018-2025 mark
# distribution (mean 4.13, median 4, max 9), not ported from FPM or Maths B.
DIFFICULTY_BANDS = ((3, "easy"), (5, "medium"))  # else "hard"
HARD_LABEL = "hard"
...
BOILERPLATE_PATTERNS = (
    ...
    re.compile(r"Mathematics A\s*[·|].*", re.I),         # GradeMax stamp line 1
    re.compile(r"4MA\d\s*\|.*", re.I),                   # GradeMax/PMT stamp line 2
...
# Common words in Maths A question stems, used only to judge whether text is
# readable and whether a repair improved it. Weighted toward this syllabus --
# bounds, surds, similar shapes and cumulative frequency barely appear in FPM,
# and Maths B's matrix/determinant vocabulary is not 4MA1 content at all.
EXAM_WORDS = frozenset(
    ...
    universal element subset inverse transformation
    ...
    expand nearest degrees centimetres metres speed distance time
    surd rationalise denominator indices index power root bound bounds upper
    lower significant figures recurring decimal proportional inversely directly
    varies quartile interquartile density pressure cone sphere cylinder prism
    pyramid frustum arc sector segment congruent similar sine cosine elevation
    depression rearrange subject inequality inequalities region shaded integer
    prime highest common multiple lowest standard notation cumulative""".split()
)
...
def extract_stem(pdf_path: Path) -> tuple[str, str]:
    ...
    # Maths A stems are genuinely short -- "Factorise 6x + 9x - 4" is a
...
def main() -> int:
    ...
    if not WORKBOOK_DIR.is_dir():
        print(
            ...
            f"Run build_mathsa_workbook_segments.py --execute first."
        )
        return 1
    ...
    print(f"{'=' * 74}\nMATHS A (4MA1) HIGHER WORKBOOK ENRICHMENT\n{'=' * 74}")
